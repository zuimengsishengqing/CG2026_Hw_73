...
NODE_DECLARATION_FUNCTION(rasterize_impl)
{
    ...
    // Displacement parameters for real-time adjustment
    b.add_input<float>("Displacement Scale")
        .default_val(1.0f)
        .min(0.0f)
        .max(5.0f);
    b.add_input<float>("Displacement Bias")
        .default_val(0.0f)
        .min(-1.0f)
        .max(1.0f);
...
NODE_EXECUTION_FUNCTION(rasterize_impl)
{
    try {
        ...
        // Set displacement parameters from node inputs
        auto dispScale = params.get_input<float>("Displacement Scale");
        auto dispBias = params.get_input<float>("Displacement Bias");
        shader_handle->shader.setFloat("displacementScale", dispScale);
        shader_handle->shader.setFloat("displacementBias", dispBias);
