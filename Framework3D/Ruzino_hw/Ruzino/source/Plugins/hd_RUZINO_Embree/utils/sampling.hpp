...
inline float GGX(float cosTheta, float alpha)
{
    ...
    return alphaSqr / (M_PI * cosThetaSqr * cosThetaSqr *
                       (alphaSqr + cosThetaSqr) * (alphaSqr + cosThetaSqr));
}

inline GfVec3f
GGXWeightedDirection(const GfVec2f& uniform_float, float roughness, float& pdf)
{
    float alpha = roughness * roughness;
    float phi = 2.0 * M_PI * uniform_float[0];
    float cosTheta = sqrt(
        (1.0 - uniform_float[1]) /
        (1.0 + (alpha * alpha - 1.0) * uniform_float[1]));
