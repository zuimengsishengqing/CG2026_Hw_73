    ...
    while (*aValue != '\0') {
        ...
        if (*aValue == '\r') {
            ...
            if (cindex < (int)mLines[aWhere.mLine].size()) {
                ...
                newLine.insert(
                    newLine.begin(), line.begin() + cindex, line.end());
            ...
            if (line[columnIndex].mChar == '\t') {
                float spaceSize =
                    ImGui::GetFont()
                        ...
                        .x;
                float oldX = columnX;
                float newColumnX = (1.0f + std::floor(
                                               (1.0f + columnX) /
                ...
                buf[i] = '\0';
                columnWidth = ImGui::GetFont()
                                  ->CalcTextSizeA(
                                      ImGui::GetFontSize(), FLT_MAX, -1.0f, buf)
                                  .x;
                if (mTextStart + columnX + columnWidth * 0.5f > local.x)
                    break;
                columnX += columnWidth;
...
TextEditor::Coordinates TextEditor::FindNextWord(const Coordinates& aFrom) const
{
    ...
    while (!isword || skip) {
        ...
        if (cindex < (int)line.size()) {
            ...
            if (isword && !skip)
                return Coordinates(
                    at.mLine, GetCharacterColumn(at.mLine, cindex));
...
void TextEditor::HandleKeyboardInputs()
{
    ...
    if (ImGui::IsWindowFocused()) {
        ...
        if (!IsReadOnly() && ctrl && !shift && !alt &&
            ...
            !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_LeftArrow)))
            ...
            !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_PageUp)))
            MoveUp(GetPageSize() - 4, shift);
        else if (
            !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_PageDown)))
            ...
            */
            ...
            if (tripleClick) {
                if (!ctrl) {
                    ...
                    SetSelection(
                        mInteractiveStart, mInteractiveEnd, mSelectionMode);
                }
...
void TextEditor::Render()
{
    ...
    const float fontSize =
        ImGui::GetFont()
            ->CalcTextSizeA(
                ImGui::GetFontSize(), FLT_MAX, -1.0f, "#", nullptr, nullptr)
            .x;
    mCharAdvance =
    ...
    mTextStart =
        ImGui::GetFont()
            ->CalcTextSizeA(
                ImGui::GetFontSize(), FLT_MAX, -1.0f, buf, nullptr, nullptr)
            .x +
        mLeftMargin;
    ...
    if (!mLines.empty()) {
        ...
        while (lineNo <= lineMax) {
            ...
            longest = std::max(
                mTextStart + TextDistanceToLineStart(
            ...
            if (mState.mSelectionEnd > lineStartCoord)
                ssend = TextDistanceToLineStart(
                    mState.mSelectionEnd < lineEndCoord ? mState.mSelectionEnd
                                                        : lineEndCoord);
            ...
            if (sstart != -1 && ssend != -1 && sstart < ssend) {
                ImVec2 vstart(
                    lineStartScreenPos.x + mTextStart + sstart,
                    lineStartScreenPos.y);
                ImVec2 vend(
                    lineStartScreenPos.x + mTextStart + ssend,
                    lineStartScreenPos.y + mCharAdvance.y);
                drawList->AddRectFilled(
                    vstart, vend, mPalette[(int)PaletteIndex::Selection]);
            }

            // Draw breakpoints
            auto start =
                ImVec2(lineStartScreenPos.x + scrollX, lineStartScreenPos.y);
            ...
            if (mBreakpoints.count(lineNo + 1) != 0) {
                auto end = ImVec2(
                    ...
                    lineStartScreenPos.y + mCharAdvance.y);
                drawList->AddRectFilled(
                    start, end, mPalette[(int)PaletteIndex::Breakpoint]);
            }
            ...
            if (errorIt != mErrorMarkers.end()) {
                ...
                if (ImGui::IsMouseHoveringRect(lineStartScreenPos, end)) {
                    ...
                    ImGui::Separator();
                    ImGui::PushStyleColor(
                        ImGuiCol_Text, ImVec4(1.0f, 1.0f, 0.2f, 1.0f));
            ...
            drawList->AddText(
                ImVec2(
                    lineStartScreenPos.x + mTextStart - lineNoWidth,
                    lineStartScreenPos.y),
            ...
            if (mState.mCursorPosition.mLine == lineNo) {
                ...
                if (focused) {
                    ...
                    if (elapsed > 400) {
                        ...
                        ImVec2 cstart(
                            textScreenPos.x + cx, lineStartScreenPos.y);
                        ImVec2 cend(
                            textScreenPos.x + cx + width,
                            lineStartScreenPos.y + mCharAdvance.y);
                        drawList->AddRectFilled(
                            cstart, cend, mPalette[(int)PaletteIndex::Cursor]);
                        if (elapsed > 800)
                            mStartTime = timeEnd;
                    }
                }
            ...
            for (int i = 0; i < line.size();) {
                ...
                if ((color != prevColor || glyph.mChar == '\t' ||
                     glyph.mChar == ' ') &&
                    !mLineBuffer.empty()) {
                    ...
                    drawList->AddText(
                        newOffset, prevColor, mLineBuffer.c_str());
                ...
                if (glyph.mChar == '\t') {
                    ...
                    if (mShowWhitespaces) {
                        ...
                        const auto y =
                            textScreenPos.y + bufferOffset.y + s * 0.5f;
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
    ...
    if (HasSelection()) {
        if (aChar == '\t' &&
                ...
                if (aShift) {
                    if (!line.empty()) {
                        if (line.front().mChar == '\t') {
                            ...
                            for (int j = 0; j < mTabSize && !line.empty() &&
                                            line.front().mChar == ' ';
                                 j++) {
                                line.erase(line.begin());
                                modified = true;
            ...
            if (modified) {
                start = Coordinates(
                    start.mLine, GetCharacterColumn(start.mLine, 0));
                ...
                if (originalEnd.mColumn != 0) {
                    ...
                    rangeEnd = Coordinates(
                        end.mLine - 1, GetLineMaxColumn(end.mLine - 1));
    ...
    if (aChar == '\n') {
        ...
        if (e > 0) {
            ...
            for (auto p = buf; *p != '\0'; p++, ++cindex)
                line.insert(
                    line.begin() + cindex, Glyph(*p, PaletteIndex::Default));
            u.mAdded = buf;
    ...
    switch (aMode) {
        ...
        case TextEditor::SelectionMode::Word: {
            ...
            if (!IsOnWordBoundary(mState.mSelectionEnd))
                mState.mSelectionEnd =
                    FindWordEnd(FindWordStart(mState.mSelectionEnd));
            break;
        }
        case TextEditor::SelectionMode::Line: {
            ...
            mState.mSelectionStart =
                Coordinates(mState.mSelectionStart.mLine, 0);
            mState.mSelectionEnd =
                Coordinates(lineNo, GetLineMaxColumn(lineNo));
            break;
        }
...
void TextEditor::MoveDown(int aAmount, bool aSelect)
{
    ...
    mState.mCursorPosition.mLine = std::max(
        0,
        std::min(
            (int)mLines.size() - 1, mState.mCursorPosition.mLine + aAmount));
    ...
    while (aAmount-- > 0) {
        if (cindex == 0) {
            ...
            if (cindex > 0) {
                if ((int)mLines.size() > line) {
                    while (cindex > 0 &&
                           IsUTFSequence(mLines[line][cindex].mChar))
                        --cindex;
                }
            }
        }
...
void TextEditor::MoveEnd(bool aSelect)
{
    auto oldPos = mState.mCursorPosition;
    SetCursorPosition(Coordinates(
        mState.mCursorPosition.mLine, GetLineMaxColumn(oldPos.mLine)));
    ...
    if (HasSelection()) {
        ...
        if (mState.mCursorPosition.mColumn == 0) {
            ...
            for (auto& i : mErrorMarkers)
                etmp.insert(
                    ErrorMarkers::value_type(
                        i.first - 1 == mState.mCursorPosition.mLine
                            ? i.first - 1
                            : i.first,
                        i.second));
            mErrorMarkers = std::move(etmp);
        ...
        for (auto first = bufferBegin; first != last;) {
            ...
            if (mLanguageDefinition.mTokenize != nullptr) {
                if (mLanguageDefinition.mTokenize(
                        first, last, token_begin, token_end, token_color))
            ...
            if (hasTokenizeResult == false) {
                ...
                if (token_color == PaletteIndex::Identifier) {
                    ...
                    // todo : allmost all language definitions use lower case to
                    // specify keywords, so shouldn't this use ::tolower ?
                    ...
                    if (!line[first - bufferBegin].mPreprocessor) {
                        if (mLanguageDefinition.mKeywords.count(id) != 0)
                            ...
                            mLanguageDefinition.mIdentifiers.count(id) != 0)
                            token_color = PaletteIndex::KnownIdentifier;
                        else if (
                            mLanguageDefinition.mPreprocIdentifiers.count(id) !=
                            0)
                            token_color = PaletteIndex::PreprocIdentifier;
                    }
                    else {
                        if (mLanguageDefinition.mPreprocIdentifiers.count(id) !=
                            0)
                            token_color = PaletteIndex::PreprocIdentifier;
                    }
                }
...
void TextEditor::ColorizeInternal()
{
    ...
    if (mCheckComments) {
        ...
        auto firstChar = true;  // there is no other non-whitespace characters
                                // in the line before
            ...
            if (!line.empty()) {
                ...
                if (withinString) {
                    ...
                    if (c == '\"') {
                        ...
                        auto& singleStartStr =
                            mLanguageDefinition.mSingleLineComment;
...
void TextEditor::UndoRecord::Undo(TextEditor* aEditor)
{
    if (!mAdded.empty()) {
        ...
        aEditor->Colorize(
            mAddedStart.mLine - 1, mAddedEnd.mLine - mAddedStart.mLine + 2);
    }
...
void TextEditor::UndoRecord::Redo(TextEditor* aEditor)
{
    ...
    if (!mAdded.empty()) {
        ...
        aEditor->Colorize(
            mAddedStart.mLine - 1, mAddedEnd.mLine - mAddedStart.mLine + 1);
    }
...
TextEditor::LanguageDefinition::CPlusPlus()
{
    ...
    if (!inited) {
        ...
        langDef.mTokenize = [](const char* in_begin,
            ...
            while (in_begin < in_end && isascii(*in_begin) &&
                   isblank(*in_begin))
                in_begin++;
            ...
            else if (TokenizeCStyleCharacterLiteral(
                         in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::CharLiteral;
            else if (TokenizeCStyleIdentifier(
                         in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Identifier;
            else if (TokenizeCStyleNumber(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Number;
            else if (TokenizeCStylePunctuation(
                         in_begin, in_end, out_begin, out_end))
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::XML()
{
    ...
    if (!inited) {
        ...
        // String patterns for attributes
        langDef.mTokenRegexStrings.push_back(
            std::make_pair<std::string, PaletteIndex>(
                "\\\"[^\\\"]*\\\"", PaletteIndex::String));
        langDef.mTokenRegexStrings.push_back(
