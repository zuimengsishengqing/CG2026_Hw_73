...
// Define template specializations of Hd_RUZINO_TypeHelper methods for
// all our supported types...
#define TYPE_HELPER(T,type)\
template<> inline HdTupleType \
Hd_RUZINO_TypeHelper::GetTupleType<T>() { return HdTupleType{type, 1}; }
...
public:
    ...
    Hd_RUZINO_BufferSampler(HdVtBufferSource const& buffer)
        : _buffer(buffer) {}
    ...
    template<typename T> bool Sample(int index, T* value) const {
        return Sample(index, static_cast<void*>(value),
            Hd_RUZINO_TypeHelper::GetTupleType<T>());
    }
...
/// \class Hd_RUZINO_PrimvarSampler
///
...
public:
    /// Default constructor.
    Hd_RUZINO_PrimvarSampler() = default;
    /// Default destructor.
    virtual ~Hd_RUZINO_PrimvarSampler() = default;
