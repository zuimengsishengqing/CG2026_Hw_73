...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    (evalStencils)(stencilData)(sizes)(offsets)(indices)(weights)(baseFaceToRefinedFacesMap)(refinedFaceCounts)(fvarIndices)(fvarPatchParam));
...
bool HdSt_OsdStencilTableBufferSource::Resolve()
{
...
bool HdSt_OsdStencilTableBufferSource::_CheckValid() const
{
    return true;
}
    ...
    pxr::HdBufferSpecVector *specs) const
{
    if (_topology->RefinesToBSplinePatches()) {
        ...
        specs->emplace_back(
            HdTokens->primitiveParam, HdTupleType{ HdTypeInt32Vec4, 1 });
        specs->emplace_back(
        ...
        // triangles (loop)
        specs->emplace_back(
            HdTokens->indices, HdTupleType{ HdTypeInt32Vec3, 1 });
        specs->emplace_back(
            HdTokens->primitiveParam, HdTupleType{ HdTypeInt32Vec3, 1 });
        // int will suffice, but this unifies it for all the cases
        specs->emplace_back(
            HdTokens->edgeIndices, HdTupleType{ HdTypeInt32Vec2, 1 });
    }
        ...
        if (_topology->TriangulateQuads()) {
            specs->emplace_back(
                HdTokens->indices, HdTupleType{ HdTypeInt32, 6 });
        }
        else {
            specs->emplace_back(
                HdTokens->indices, HdTupleType{ HdTypeInt32, 4 });
        }
        specs->emplace_back(
            HdTokens->primitiveParam, HdTupleType{ HdTypeInt32Vec3, 1 });
        specs->emplace_back(
...
/*virtual*/
bool HdSt_OsdIndexComputation::HasChainedBuffer() const
{
    return true;
}

/*virtual*/
HdBufferSourceSharedPtrVector HdSt_OsdIndexComputation::GetChainedBuffers()
    const
{
...
/*virtual*/
bool HdSt_OsdIndexComputation::_CheckValid() const
{
    return true;
}
...
size_t HdSt_OsdRefineComputationCPU::ComputeHash() const
{
    return TfHash()(*this);
}
...
HdTupleType HdSt_OsdRefineComputationCPU::GetTupleType() const
{
    return _source->GetTupleType();
}

size_t HdSt_OsdRefineComputationCPU::GetNumElements() const
{
...
bool HdSt_OsdRefineComputationCPU::Resolve()
{
    ...
    subdivision->RefineCPU(
        _source, &_primvarBuffer, _interpolation, _fvarChannel);
...
bool HdSt_OsdRefineComputationCPU::_CheckValid() const
{
...
bool HdSt_OsdRefineComputationCPU::HasPreChainedBuffer() const
{
    return true;
}

HdBufferSourceSharedPtr HdSt_OsdRefineComputationCPU::GetPreChainedBuffer()
    const
{
    return _source;
}
...
int HdSt_Subdivision::GetNumVertices() const
{
...
int HdSt_Subdivision::GetNumVarying() const
{
    // returns the total number of vertices, including coarse and refined ones.
...
int HdSt_Subdivision::GetNumFaceVarying(int channel) const
{
    // returns the total number of vertices, including coarse and refined ones.
...
int HdSt_Subdivision::GetMaxNumFaceVarying() const
{
...
VtIntArray HdSt_Subdivision::GetRefinedFvarIndices(int channel) const
{
...
bool HdSt_OsdTopologyComputation::Resolve()
{
    ...
    if (_topology->GetFaceVertexCounts().size() == 0) {
        ...
        refiner = PxOsdRefinerFactory::Create(
            _topology->GetPxOsdMeshTopology(),
    ...
    subdivision->SetRefinementTables(
        std::move(vertexStencils),
...
bool HdSt_OsdTopologyComputation::_CheckValid() const
{
    return true;
}
...
bool HdSt_OsdIndexComputation::Resolve()
{
    using namespace OpenSubdiv;
    ...
    if (_topology->RefinesToBSplinePatches() ||
        ...
        HdBufferSourceSharedPtr triIndices = std::make_shared<HdVtBufferSource>(
    ...
    for (int i = 0, v = 0, ev = 0; i < numFaces; ++i) {
        ...
        if (nv == regFaceSize) {
            ...
            for (int j = 0; j < nv; ++j) {
                ...
                info.baseFaceEdgeIndices =
                    GfVec2i(ev + j, ev + (j + nv - 1) % nv);
                result->push_back(info);
            }
        }
    ...
    HdSt_Subdivision::PatchTable const *patchTable)
{
    ...
    _edgeIndicesBuffer.reset(
        new HdVtBufferSource(HdTokens->edgeIndices, VtValue(edgeIndices)));
}
...
{
    _indicesName =
        TfToken(_tokens->fvarIndices.GetString() + std::to_string(_channel));
    _patchParamName =
        TfToken(_tokens->fvarPatchParam.GetString() + std::to_string(_channel));
}

bool HdSt_OsdFvarIndexComputation::Resolve()
{
    using namespace OpenSubdiv;
    ...
    if (_topology->RefinesToBSplinePatches() ||
        ...
        memcpy(
            indices.data(), firstIndex, arraySize * numPatches * sizeof(int));
    ...
    HdSt_Subdivision::PatchTable const *patchTable)
{
    ...
    _fvarPatchParamBuffer.reset(
        new HdVtBufferSource(_patchParamName, VtValue(fvarPatchParam)));
}
...
bool HdSt_OsdFvarIndexComputation::HasChainedBuffer() const
{
    return (
        _topology->RefinesToBSplinePatches() ||
        _topology->RefinesToBoxSplineTrianglePatches());
}

HdBufferSourceSharedPtrVector HdSt_OsdFvarIndexComputation::GetChainedBuffers()
    const
{
    if (_topology->RefinesToBSplinePatches() ||
        _topology->RefinesToBoxSplineTrianglePatches()) {
...
bool HdSt_OsdFvarIndexComputation::_CheckValid() const
{
    return true;
}
    ...
    pxr::HdBufferSpecVector *specs) const
{
    ...
    specs->emplace_back(
        _tokens->refinedFaceCounts, HdTupleType{ HdTypeInt32, 1 });
}

bool HdSt_OsdBaseFaceToRefinedFacesMapComputation::Resolve()
{
    if (_osdTopology && !_osdTopology->IsResolved())
        return false;
    ...
    const size_t numPatches =
        patchTable ? patchTable->GetPatchParamTable().size() : 0;
    const size_t numBaseFaces = patchTable ? patchTable->GetNumPtexFaces() : 0;
...
bool HdSt_OsdBaseFaceToRefinedFacesMapComputation::HasChainedBuffer() const
{
    return true;
}
...
bool HdSt_OsdBaseFaceToRefinedFacesMapComputation::_CheckValid() const
{
    return true;
}
