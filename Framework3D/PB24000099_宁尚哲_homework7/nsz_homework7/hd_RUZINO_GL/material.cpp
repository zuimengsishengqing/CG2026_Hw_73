...
void Hd_RUZINO_Material::DestroyTexture(InputDescriptor& input_descriptor)
{
    ...
    HdMaterialNode2& usd_preview_surface)
{
    for (auto&& input_connection : usd_preview_surface.inputConnections) {
        if (input_connection.first == TfToken(name)) {
            ...
            descriptor.image =
                HioImage::OpenForReading(file_name, 0, 0, colorSpace);
            descriptor.wrapS =
...
void Hd_RUZINO_Material::TryCreateGLTexture(InputDescriptor& descriptor)
{
...
void Hd_RUZINO_Material::RefreshGLBuffer()
{
    ;
...
void Hd_RUZINO_Material::BindTextures(Shader& shader)
{
    ...
    // Bind displacement map (unit 3)
    shader.setInt("displacementSampler", 3);
    glActiveTexture(GL_TEXTURE3);
    glBindTexture(GL_TEXTURE_2D, displacement.glTexture);

    // Set displacement parameters from material (can be overridden by node parameters)
    float dispScale = 1.0f;
    float dispBias = 0.0f;
    if (!displacement.value.IsEmpty()) {
        if (displacement.value.CanCast<float>()) {
            dispScale = displacement.value.Get<float>();
        }
        else if (displacement.value.CanCast<GfVec3f>()) {
            auto v = displacement.value.Get<GfVec3f>();
            dispScale = v[0];
        }
    }
    // Note: These values can be overridden by node parameters in rasterize_impl
    shader.setFloat("displacementScale", dispScale);
    shader.setFloat("displacementBias", dispBias);
}
