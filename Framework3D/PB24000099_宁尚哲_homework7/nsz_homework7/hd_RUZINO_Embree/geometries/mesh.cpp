...
void Hd_RUZINO_Mesh::_CreatePrimvarSampler(
    const TfToken& name,
    const VtValue& data,
    ...
    bool refined)
{
    ...
    Hd_RUZINO_PrototypeContext* ctx = _GetPrototypeContext();
    ...
    // geometry mode.
    Hd_RUZINO_PrimvarSampler* sampler = nullptr;
    switch (interpolation) {
        case HdInterpolationConstant:
            sampler = new Hd_RUZINO_ConstantSampler(name, data);
            break;
        case HdInterpolationUniform:
            if (refined) {
                sampler = new Hd_RUZINO_UniformSampler(name, data);
            }
            else {
                sampler = new Hd_RUZINO_UniformSampler(
                    name, data, _trianglePrimitiveParams);
            }
            break;
        case HdInterpolationVertex:
            if (refined) {
                sampler = new Hd_RUZINO_SubdivVertexSampler(
                    name,
                    data,
                    _rtcMeshScene,
                    _rtcMeshId,
                ...
                sampler = new Hd_RUZINO_TriangleVertexSampler(
        ...
        case HdInterpolationFaceVarying:
            if (refined) {
                // XXX: Fixme! Hd_RUZINO_ doesn't currently support
                // face-varying primvars on subdivision meshes.
                TF_WARN(
                    "Hd_RUZINO_Mesh doesn't support face-varying primvars"
                    " on refined meshes.");
            }
            else {
                HdMeshUtil meshUtil(&_topology, GetId());
                sampler = new Hd_RUZINO_TriangleFaceVaryingSampler(
                    name, data, meshUtil);
            }
            break;
...
RTCGeometry Hd_RUZINO_Mesh::_CreateEmbreeSubdivMesh(
    RTCScene scene,
    RTCDevice device)
{
        ...
        for (size_t i = 0; i < creaseLengths.size(); ++i) {
            ...
            for (int j = 0; j < numEdges; ++j) {
                ...
                embreeCreaseWeights[embreeEdgeIndex] =
                    weightPerCrease ? creaseWeights[i]
...
RTCGeometry Hd_RUZINO_Mesh::_CreateEmbreeTriangleMesh(
    RTCScene scene,
    RTCDevice device)
{
...
void Hd_RUZINO_Mesh::_PopulateRtMesh(
    HdSceneDelegate* sceneDelegate,
    RTCScene scene,
    RTCDevice device,
    HdDirtyBits* dirtyBits,
    const HdMeshReprDesc& desc)
{
    ...
    if (HdChangeTracker::IsTopologyDirty(*dirtyBits, id) ||
        ...
        rtcSetGeometryUserData(_geometry, new Hd_RUZINO_PrototypeContext);
    ...
    if (newMesh || HdChangeTracker::IsDisplayStyleDirty(*dirtyBits, id)) {
        if (doRefine) {
            ...
            rtcSetGeometryTessellationRate(
                _geometry, static_cast<float>(tessellationRate));
        }
    }
        ...
        if (doRefine) {
            ...
            if (vertexRule == PxOsdOpenSubdivTokens->none) {
                rtcSetGeometrySubdivisionMode(
                    _geometry, 0, RTC_SUBDIVISION_MODE_NO_BOUNDARY);
            }
            else if (vertexRule == PxOsdOpenSubdivTokens->edgeOnly) {
                rtcSetGeometrySubdivisionMode(
                    _geometry, 0, RTC_SUBDIVISION_MODE_SMOOTH_BOUNDARY);
            }
            else if (vertexRule == PxOsdOpenSubdivTokens->edgeAndCorner) {
                rtcSetGeometrySubdivisionMode(
                    _geometry, 0, RTC_SUBDIVISION_MODE_PIN_CORNERS);
            }
            else {
    ...
    TF_FOR_ALL(it, _primvarSourceMap)
    {
        if (newMesh ||
            ...
            _CreatePrimvarSampler(
                it->first, it->second.data, it->second.interpolation, _refined);
        }
    }
    ...
    if (HdChangeTracker::IsInstancerDirty(*dirtyBits, id) ||
        ...
        if (!GetInstancerId().IsEmpty()) {
            ...
            transforms = static_cast<Hd_RUZINO_Instancer*>(instancer)
        ...
        for (size_t i = oldSize; i < newSize; ++i) {
            ...
            auto ctx = new Hd_RUZINO_InstanceContext;
...
/* static */
void Hd_RUZINO_Mesh::_EmbreeCullFaces(const RTCFilterFunctionNArguments* args)
{
    if (!args) {
    ...
    auto ctx = static_cast<Hd_RUZINO_PrototypeContext*>(args->geometryUserPtr);
    if (!ctx || !ctx->rprim) {
        TF_CODING_ERROR("_EmbreeCullFaces got NULL prototype context");
        return;
    }
    auto mesh = static_cast<Hd_RUZINO_Mesh*>(ctx->rprim);
...
Hd_RUZINO_PrototypeContext* Hd_RUZINO_Mesh::_GetPrototypeContext()
{
    return static_cast<Hd_RUZINO_PrototypeContext*>(
        rtcGetGeometryUserData(_geometry));
}

Hd_RUZINO_InstanceContext* Hd_RUZINO_Mesh::_GetInstanceContext(
    RTCScene scene,
    size_t i)
{
    return static_cast<Hd_RUZINO_InstanceContext*>(
    ...
    const TfToken& reprToken)
{
    ...
    // For now, Hd_RUZINO_Mesh only respects the first desc; this should be
    // fixed.
    _MeshReprConfig::DescArray descs = _GetReprDesc(reprToken);
    const HdMeshReprDesc& desc = descs[0];

    // Pull top-level embree state out of the render param.
    auto embreeRenderParam = static_cast<Hd_RUZINO_RenderParam*>(renderParam);
