...
#endif
#include "blueprints/imgui_canvas.h"
    ...
    class __trait_name__ {                                                \
       ...
       public:                                                            \
        static constexpr bool value =                                     \
            (sizeof(yes_type) == sizeof(test<mixin>(0)));                 \
    }

// Special sentinel value. This needs to be unique, so allow it to be overridden
// in the user's ImGui config
...
struct FringeScaleRef {
    // Overload is present when ImDrawList does have _FringeScale member
    // variable.
    template<typename T>
    static float& Get(
        typename std::enable_if<HasFringeScale<T>::value, T>::type* drawList)
    {
        return drawList->_FringeScale;
    }

    // Overload is present when ImDrawList does not have _FringeScale member
    // variable.
    template<typename T>
    static float& Get(
...
struct VtxCurrentOffsetRef {
    ...
    static unsigned int& Get(
        typename std::enable_if<HasVtxCurrentOffset<T>::value, T>::type*
            drawList)
    {
...
bool ImGuiEx::Canvas::Begin(ImGuiID id, const ImVec2& size)
{
    ...
    // m_DrawList->AddRectFilled(m_StartPos, m_StartPos + m_CurrentSize,
    // IM_COL32(0, 0, 0, 64)); m_DrawList->AddRect(m_WidgetRect.Min,
    // m_WidgetRect.Max, IM_COL32(255, 0, 255, 64));
...
void ImGuiEx::Canvas::End()
{
    ...
    // m_DrawList->AddRect(m_WidgetPosition - ImVec2(1.0f, 1.0f),
    // m_WidgetPosition + m_WidgetSize + ImVec2(1.0f, 1.0f), IM_COL32(196, 0, 0,
    // 255));
...
ImGuiEx::CanvasView ImGuiEx::Canvas::CalcCenterView(
    const ImVec2& canvasPoint) const
{
    ...
    const ImRect& canvasRect) const
{
    ...
    auto widgetAspectRatio =
        m_WidgetSize.y > 0.0f ? m_WidgetSize.x / m_WidgetSize.y : 0.0f;
    auto canvasRectAspectRatio =
        canvasRectSize.y > 0.0f ? canvasRectSize.x / canvasRectSize.y : 0.0f;
...
void ImGuiEx::Canvas::Resume()
{
    ...
    // Check: Number of calls to Resume() do not match calls to Suspend().
...
ImVec2 ImGuiEx::Canvas::FromLocal(const ImVec2& point, const CanvasView& view)
    const
{
...
ImVec2 ImGuiEx::Canvas::FromLocalV(const ImVec2& vector, const CanvasView& view)
    const
{
...
ImVec2 ImGuiEx::Canvas::ToLocal(const ImVec2& point, const CanvasView& view)
    const
{
...
ImVec2 ImGuiEx::Canvas::ToLocalV(const ImVec2& vector, const CanvasView& view)
    const
{
...
void ImGuiEx::Canvas::EnterLocalSpace()
{
    ...
    ImGui::PushClipRect(
        m_WidgetPosition, m_WidgetPosition + m_WidgetSize, true);
    ...
    m_CurrentRange->BeginVertexIndex =
        m_DrawList->_VtxCurrentIdx + ImVtxOffsetRef(m_DrawList);
#endif
    m_DrawListCommadBufferSize = ImMax(m_DrawList->CmdBuffer.Size, 0);
    m_DrawListStartVertexIndex =
        m_DrawList->_VtxCurrentIdx + ImVtxOffsetRef(m_DrawList);
    ...
    if ((!m_DrawList->CmdBuffer.empty() &&
         m_DrawList->CmdBuffer.back().ElemCount > 0) ||
        m_DrawList->_Splitter._Count > 1)
    ...
    viewport_min.x =
        (viewport_min.x - m_ViewTransformPosition.x) * m_View.InvScale;
    viewport_min.y =
        (viewport_min.y - m_ViewTransformPosition.y) * m_View.InvScale;
    viewport_max.x =
        (viewport_max.x - m_ViewTransformPosition.x) * m_View.InvScale;
    viewport_max.y =
        (viewport_max.y - m_ViewTransformPosition.y) * m_View.InvScale;
    ...
    clipped_clip_rect.x =
        (clipped_clip_rect.x - m_ViewTransformPosition.x) * m_View.InvScale;
    clipped_clip_rect.y =
        (clipped_clip_rect.y - m_ViewTransformPosition.y) * m_View.InvScale;
    clipped_clip_rect.z =
        (clipped_clip_rect.z - m_ViewTransformPosition.x) * m_View.InvScale;
    clipped_clip_rect.w =
        (clipped_clip_rect.w - m_ViewTransformPosition.y) * m_View.InvScale;
    ImGui::PushClipRect(
        ImVec2(clipped_clip_rect.x, clipped_clip_rect.y),
        ImVec2(clipped_clip_rect.z, clipped_clip_rect.w),
        false);

    // Transform mouse position to local space.
    auto& io = ImGui::GetIO();
    io.MousePos =
        (m_MousePosBackup - m_ViewTransformPosition) * m_View.InvScale;
    io.MousePosPrev =
        (m_MousePosPrevBackup - m_ViewTransformPosition) * m_View.InvScale;
    for (auto i = 0; i < IM_ARRAYSIZE(m_MouseClickedPosBackup); ++i)
        io.MouseClickedPos[i] =
            (m_MouseClickedPosBackup[i] - m_ViewTransformPosition) *
            m_View.InvScale;
...
void ImGuiEx::Canvas::LeaveLocalSpace()
{
    ...
    auto vertexEnd = m_DrawList->VtxBuffer.Data + m_DrawList->_VtxCurrentIdx +
                     ImVtxOffsetRef(m_DrawList);

    // If canvas view is not scaled take a faster path.
    if (m_View.Scale != 1.0f) {
        while (vertex < vertexEnd) {
            vertex->pos.x =
                vertex->pos.x * m_View.Scale + m_ViewTransformPosition.x;
            vertex->pos.y =
                vertex->pos.y * m_View.Scale + m_ViewTransformPosition.y;
            ++vertex;
        }

        // Move clip rectangles to screen space.
        for (int i = m_DrawListFirstCommandIndex;
             i < m_DrawList->CmdBuffer.size();
             ++i) {
            auto& command = m_DrawList->CmdBuffer[i];
            command.ClipRect.x =
                command.ClipRect.x * m_View.Scale + m_ViewTransformPosition.x;
            command.ClipRect.y =
                command.ClipRect.y * m_View.Scale + m_ViewTransformPosition.y;
            command.ClipRect.z =
                command.ClipRect.z * m_View.Scale + m_ViewTransformPosition.x;
            command.ClipRect.w =
                command.ClipRect.w * m_View.Scale + m_ViewTransformPosition.y;
        }
    }
    else {
        while (vertex < vertexEnd) {
    ...
    if (m_DrawListCommadBufferSize > 0) {
        if (m_DrawList->CmdBuffer.size() > m_DrawListCommadBufferSize &&
            m_DrawList->CmdBuffer[m_DrawListCommadBufferSize].UserCallback ==
                ImDrawCallback_ImCanvas)
            m_DrawList->CmdBuffer.erase(
                m_DrawList->CmdBuffer.Data + m_DrawListCommadBufferSize);
        else if (
            m_DrawList->CmdBuffer.size() >= m_DrawListCommadBufferSize &&
            m_DrawList->CmdBuffer[m_DrawListCommadBufferSize - 1]
                    .UserCallback == ImDrawCallback_ImCanvas)
            m_DrawList->CmdBuffer.erase(
                m_DrawList->CmdBuffer.Data + m_DrawListCommadBufferSize - 1);
    }
