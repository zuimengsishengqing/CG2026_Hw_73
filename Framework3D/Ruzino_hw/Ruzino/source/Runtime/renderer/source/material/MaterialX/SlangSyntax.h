...
class HD_RUZINO_API SlangSyntax : public Syntax {
   public:
    ...
    {
        return std::make_shared<SlangSyntax>(typeSystem);
    }

    const string& getInputQualifier() const override
    {
        return INPUT_QUALIFIER;
    }
    ...
    {
        return CONSTANT_QUALIFIER;
    };
    const string& getUniformQualifier() const override
    {
        return UNIFORM_QUALIFIER;
    };
    ...
    /// converted to a different type than the input.
    bool remapEnumeration(
        const string& value,
        TypeDesc type,
        const string& enumNames,
        std::pair<TypeDesc, ValuePtr>& result) const override;
