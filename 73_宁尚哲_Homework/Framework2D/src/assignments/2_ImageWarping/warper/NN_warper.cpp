...
void NNWarper::set_control_points(const std::vector<std::pair<float, float>>& start_points,
...
void NNWarper::prepare_training_data(std::vector<dlib::matrix<float>>& inputs,
                                     std::vector<dlib::matrix<float>>& targets)
{
    inputs.clear();
    targets.clear();
    ...
    std::cout << "Prepared " << inputs.size() << " training samples" << std::endl;
}

void NNWarper::train_network(int epochs, double learning_rate)
{
    if (start_points_.empty() || end_points_.empty()) {
        std::cerr << "Error: No control points set for training" << std::endl;
        return;
    }

    if (start_points_.size() != end_points_.size()) {
        std::cerr << "Error: Start and end points count mismatch" << std::endl;
        return;
    }

    std::cout << "Training neural network with " << start_points_.size() << " control points..." << std::endl;

    // 准备训练数据
    std::vector<dlib::matrix<float>> inputs;
    std::vector<dlib::matrix<float>> targets;
    prepare_training_data(inputs, targets);

    // 配置优化器
    dlib::dnn_trainer<warping_net, dlib::adam> trainer(net_);
    ...
    std::cout << "Final loss: " << trainer.get_average_loss() << std::endl;
}
...
void NNWarper::set_network_params(int hidden1_size, int hidden2_size)
{
    ...
    std::cout << "Network parameters: hidden1=" << hidden1_size << ", hidden2=" << hidden2_size << std::endl;
    is_trained_ = false;
}

std::pair<float, float> NNWarper::function(float x, float y)
{
    if (!is_trained_) {
        std::cerr << "Warning: Network not trained, returning identity mapping" << std::endl;
        return {x, y};
    }
    ...
    // 前向传播
    dlib::matrix<float> output = net_(input);
...
void NNWarper::warp(const unsigned char* src, unsigned char* dst, int width, int height)
{
    ...
    if (!is_trained_ && !start_points_.empty()) {
        std::cout << "Network not trained, training now..." << std::endl;
        train_network();
    }

    if (!is_trained_) {
        std::cerr << "Error: Cannot warp - network not trained and no control points" << std::endl;
        return;
    }
    ...
    std::cout << "Total pixels: " << total_pixels << ", Out of bounds: " << out_of_bounds_count << std::endl;
}

void NNWarper::save_network(const std::string& filename)
{
    if (!is_trained_) {
        std::cerr << "Error: Cannot save untrained network" << std::endl;
        return;
    }

    try {
        dlib::serialize(filename) << net_;
...
{
    try {
        dlib::deserialize(filename) >> net_;
        is_trained_ = true;
