...
namespace NodeEditor {
    ...
    using ConfigSaveNodeSettings = bool (*)(
        ...
        SaveReasonFlags reason,
        void* userPointer);
    using ConfigLoadNodeSettings =
        size_t (*)(NodeId nodeId, char* data, void* userPointer);
    ...
    struct Config {
        ...
        int DragButtonIndex;
        // Mouse button index drag action will react to (0-left, 1-right,
        // 2-middle)
        int SelectButtonIndex;
        // Mouse button index select action will react to (0-left, 1-right,
        // 2-middle)
        int NavigateButtonIndex;
        // Mouse button index navigate action will react to (0-left, 1-right,
        // 2-middle)
        int ContextMenuButtonIndex;
        // Mouse button index context menu action will react to (0-left,
        // 1-right, 2-middle)
    ...
    IMGUI_NODE_EDITOR_API bool
    QueryNewNode(SocketID* pinId, const ImVec4& color, float thickness = 1.0f);
    ...
    IMGUI_NODE_EDITOR_API void SetNodeZPosition(NodeId nodeId, float z);
    // Sets node z position, nodes with higher value are drawn over nodes with
    // lower value
    ...
    IMGUI_NODE_EDITOR_API bool
    GetLinkPins(LinkId linkId, SocketID* startPinId, SocketID* endPinId);
    ...
    IMGUI_NODE_EDITOR_API int GetOrderedNodeIds(NodeId* nodes, int size);
    // Fills an array with node id's in order they're drawn; up to 'size`
    // elements are set. Returns actual size of filled id's.
}  // namespace NodeEditor
}  // namespace ax
