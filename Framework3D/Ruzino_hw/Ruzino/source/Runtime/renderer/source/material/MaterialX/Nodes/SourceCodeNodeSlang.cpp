...
ShaderNodeImplPtr SourceCodeNodeSlang::create()
{
    return std::make_shared<SourceCodeNodeSlang>();
}

MaterialX::FilePath getRelative(
    const MaterialX::FilePath& path,
    const MaterialX::FilePath& searchPath)
{
    // Normalize both paths first to ensure consistent comparison
    MaterialX::FilePath normalizedPath = path.getNormalized();
    MaterialX::FilePath normalizedSearchPath = searchPath.getNormalized();
    ...
    // Convert both paths to strings with the same format for comparison
    string pathStr = normalizedPath.asString(FilePath::FormatPosix);
    string searchPathStr = normalizedSearchPath.asString(FilePath::FormatPosix);

    // Check if the path starts with the search path
    if (pathStr.find(searchPathStr) == 0) {
        // Path is within searchPath, so extract the relative part
        string relativePart = pathStr.substr(searchPathStr.length());
        ...
        return FilePath(relativePart);
    }

    // If the path isn't under the search path, return the original path
    return path;
}

void SourceCodeNodeSlang::initialize(
    ...
    GenContext& context)
{
    ...
    if (!element.isA<Implementation>()) {
        throw ExceptionShaderGenError(
            ...
            "' is not an Implementation element");
    }
    ...
    if (_functionSource.empty()) {
        FilePath localPath =
            FilePath(impl.getActiveSourceUri()).getParentPath();
        _sourceFilename =
            context.resolveSourceFile(impl.getAttribute("file"), localPath);
        _functionSource = readFile(_sourceFilename);
        if (_functionSource.empty()) {
            throw ExceptionShaderGenError(
                "Failed to get source code from file '" +
                _sourceFilename.asString() + "' used by implementation '" +
                impl.getName() + "'");
        }

        // Now make _sourceFilename relative to the search path

        FilePath current = FilePath::getCurrentPath();
    ...
    if (!_inlined) {
        ...
        context.getShaderGenerator().getSyntax().makeValidName(
            validFunctionName);
...
void SourceCodeNodeSlang::emitFunctionDefinition(
    const ShaderNode&,
    GenContext& context,
    ...
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        if (!_functionSource.empty()) {
            ...
            if (!_inlined) {
                ...
                shadergen.emitLibraryInclude(_sourceFilename, context, stage);
                // shadergen.emitBlock(
                //     _functionSource, _sourceFilename, context, stage);
...
void SourceCodeNodeSlang::emitFunctionCall(
