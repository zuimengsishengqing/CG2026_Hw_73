...
void LightCompoundNodeSlang::initialize(
...
void LightCompoundNodeSlang::createVariables(
    const ShaderNode&,
    GenContext& context,
    Shader& shader) const
{
...
void LightCompoundNodeSlang::emitFunctionDefinition(
    ...
    ShaderStage& stage) const
{
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        shadergen.emitLine(
            "void " + _functionName +
                "(LightData light, float3 position, out lightshader result)",
            stage,
            false);
        ...
        shadergen.emitLine(
            ...
            "float3(0), -L, light.direction, float3(0), 0)",
            stage);
        shadergen.emitFunctionCalls(
            *_rootGraph,
            context,
            ...
            ShaderNode::Classification::SHADER |
                ShaderNode::Classification::LIGHT);

        shadergen.emitFunctionBodyEnd(*_rootGraph, context, stage);
    }
}

void LightCompoundNodeSlang::emitFunctionCall(
    const ShaderNode&,
    GenContext& context,
    ShaderStage& stage) const
{
