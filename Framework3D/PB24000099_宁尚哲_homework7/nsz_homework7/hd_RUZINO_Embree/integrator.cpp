    ...
    const std::function<float()>& uniform_float)
{
    ...
    GfVec3f wi_brdf;
    float brdf_pdf;
    auto brdf_sample = si.Sample(wi_brdf, brdf_pdf, uniform_float);
    
    // 2. 检查采样是否有效
    if (brdf_pdf > 0 && GfDot(wi_brdf, si.shadingNormal) > 0) {
        // 3. 构造光线测试可见性
        GfRay brdf_ray(si.position + 0.0001f * si.geometricNormal, wi_brdf);
        GfVec3f intersectPos;
        
        // 4. 检查是否击中光源
        auto light_radiance = IntersectLights(brdf_ray, intersectPos);
        
        // 5. 如果击中光源，计算贡献
        if (light_radiance[0] > 0 || light_radiance[1] > 0 || light_radiance[2] > 0) {
            // 计算BRDF采样的PDF（光源采样）
            // 需要重新计算光源采样的PDF
            float light_pdf = 0;
            auto N = render_param->lights->size();
            ...
            for (auto light : (*render_param->lights)) {
                float depth = std::numeric_limits<float>::infinity();
                auto intersected_radiance = light->Intersect(brdf_ray, depth);
                if (depth < std::numeric_limits<float>::infinity() && 
                    (intersected_radiance[0] > 0 || intersected_radiance[1] > 0 || intersected_radiance[2] > 0)) {
                    // 重新计算该光源的采样PDF
                    GfVec3f dir;
                    GfVec3f sampled_light_pos_temp;
                    float sample_light_pdf_temp;
                    auto sample_light_luminance_temp = light->Sample(
                        si.position, dir, sampled_light_pos_temp, sample_light_pdf_temp, uniform_float);
                    light_pdf = sample_light_pdf_temp * select_light_pdf;
                    break;
                }
            }
            
            // 使用MIS（Multiple Importance Sampling）结合两种采样
            float weight = PowerHeuristic(brdf_pdf, light_pdf);
            float cos_theta = GfDot(wi_brdf, si.shadingNormal);
            contribution_by_sample_brdf = GfCompMult(light_radiance, brdf_sample) * cos_theta / brdf_pdf * weight;
        }
    }
    
    // 6. 使用MIS结合光源采样和BRDF采样
    // 光源采样的权重
    float light_weight = 1.0f;
    if (brdf_pdf > 0) {
        light_weight = PowerHeuristic(sample_light_pdf, brdf_pdf);
    }
    
    return contribution_by_sample_lights * light_weight + contribution_by_sample_brdf;
}
