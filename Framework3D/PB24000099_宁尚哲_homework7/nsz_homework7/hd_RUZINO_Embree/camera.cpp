...
GfRay Hd_RUZINO_Camera::generateRay(
    ...
    const std::function<float()>& uniform_float) const
{
    ...
    if (Hd_RUZINO_Config::GetInstance().jitterCamera) {
    ...
    dir = GfVec3f(_inverseViewMatrix.TransformDir(dir).GetNormalized());
    return { origin, dir };
}
...
void Hd_RUZINO_Camera::attachFilm(Hd_RUZINO_RenderBuffer* new_film) const
{
    film = new_film;
}
