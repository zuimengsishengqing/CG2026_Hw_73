...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    (coarseFaceIndex)(processedFaceCounts)(processedFaceIndices));
...
HdBufferSourceSharedPtr HdSt_MeshTopology::GetPointsIndexBuilderComputation()
{
...
bool HdSt_MeshTopology::RefinesToTriangles() const
{
    return HdSt_Subdivision::RefinesToTriangles(_topology.GetScheme());
}

bool HdSt_MeshTopology::RefinesToBSplinePatches() const
{
    return (
        (IsEnabledAdaptive() || (_refineMode == RefineModePatches)) &&
        HdSt_Subdivision::RefinesToBSplinePatches(_topology.GetScheme()));
}

bool HdSt_MeshTopology::RefinesToBoxSplineTrianglePatches() const
{
    return (
        (IsEnabledAdaptive() || (_refineMode == RefineModePatches)) &&
        HdSt_Subdivision::RefinesToBoxSplineTrianglePatches(
            _topology.GetScheme()));
}
    ...
    SdfPath const &id)
{
    ...
    bool const adaptive =
        RefinesToBSplinePatches() || RefinesToBoxSplineTrianglePatches();
        ...
        if (!faceIndices.empty() && !geomSubset.materialId.IsEmpty()) {
            ...
            for (size_t i = 0; i < faceIndices.size(); ++i) {
                ...
                if (index >= (int)numFaces) {
                    TF_WARN(
                        "Geom subset index %d is larger than number of "
                        "faces (%d), removing.",
                        index,
                        (int)numFaces);
                    continue;
                ...
                if (unusedFaces[index]) {
                    ...
                    TF_WARN(
                        "Face index %d is repeated between geom subsets",
                        index);
                    ;
                }
            }
...
HdBufferSourceSharedPtr HdSt_MeshTopology::GetIndexSubsetComputation(
    HdBufferSourceSharedPtr indexBuilderSource,
    HdBufferSourceSharedPtr faceIndicesSource)
{
...
HdBufferSourceSharedPtr HdSt_MeshTopology::GetRefinedIndexSubsetComputation(
    HdBufferSourceSharedPtr indexBuilderSource,
    HdBufferSourceSharedPtr faceIndicesSource)
{
...
HdBufferSourceSharedPtr HdSt_MeshTopology::GetOsdIndexBuilderComputation()
{
...
HdBufferSourceSharedPtr HdSt_MeshTopology::GetOsdFvarIndexBuilderComputation(
    int channel)
{
    HdBufferSourceSharedPtr topologyBuilder = _osdTopologyBuilder.lock();
    ...
    int fvarChannel)
{
    ...
    return _subdivision->CreateRefineComputationCPU(
        this, source, topologyBuilder, interpolation);
}
...
bool HdSt_IndexSubsetComputation::Resolve()
{
    if (_indexBuilderSource && !_indexBuilderSource->IsResolved())
        return false;
    if (_faceIndicesSource && !_faceIndicesSource->IsResolved())
        return false;
    ...
    if (_baseFaceToRefinedFacesMapSource) {
        ...
        for (size_t i = 0; i < faceIndices.size(); ++i) {
            const size_t start =
                faceIndices[i] == 0 ? 0 : refinedFaceCounts[faceIndices[i] - 1];
...
bool HdSt_IndexSubsetComputation::HasChainedBuffer() const
{
    return !_chainedBuffers.empty();
}

HdBufferSourceSharedPtrVector HdSt_IndexSubsetComputation::GetChainedBuffers()
    const
{
    return _chainedBuffers;
}

bool HdSt_IndexSubsetComputation::_CheckValid() const
{
    return true;
}
    ...
    HdBufferSpecVector *specs) const
{
    ...
    specs->emplace_back(
        _tokens->coarseFaceIndex, HdTupleType{ HdTypeInt32, 1 });
}

bool HdSt_GeomSubsetFaceIndexBuilderComputation::Resolve()
{
...
bool HdSt_GeomSubsetFaceIndexBuilderComputation::_CheckValid() const
{
    return true;
}
    ...
    HdBufferSpecVector *specs) const
{
    ...
    specs->emplace_back(
        _tokens->processedFaceIndices, HdTupleType{ HdTypeInt32, 1 });
}

bool HdSt_GeomSubsetFaceIndexHelperComputation::Resolve()
{
    if (!_TryLock())
        return false;
    ...
    _processedFaceIndicesBuffer.reset(new HdVtBufferSource(
        _tokens->processedFaceIndices, VtValue(processedFaceIndices)));

    _SetResolved();
    return true;
}

bool HdSt_GeomSubsetFaceIndexHelperComputation::HasChainedBuffer() const
{
    return true;
}
...
bool HdSt_GeomSubsetFaceIndexHelperComputation::_CheckValid() const
{
    return true;
}
