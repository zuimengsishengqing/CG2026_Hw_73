...
ShaderNodeImplPtr CompoundNodeSlang::create()
{
    return std::make_shared<CompoundNodeSlang>();
}

void CompoundNodeSlang::initialize(
    ...
    GenContext& context)
{
    ...
    if (!element.isA<NodeGraph>()) {
        throw ExceptionShaderGenError(
            ...
            "' is not a node graph implementation");
    }
    ...
    const ShaderInterfaceType oldShaderInterfaceType =
...
void CompoundNodeSlang::setValues(
    const Node& node,
    ShaderNode& shaderNode,
    GenContext& context) const
{
    {
        auto inputs = _rootGraph->getInputSockets();
        auto outputs = _rootGraph->getOutputSockets();

        auto shaderInputs = shaderNode.getInputs();
        auto shaderOutputs = shaderNode.getOutputs();
        ...
        for (auto shader_input : shaderInputs) {
            ...
            input->setValue(shader_input->getValue());
        }
    }

    ShaderNodeImpl::setValues(node, shaderNode, context);
}

void CompoundNodeSlang::createVariables(
    const ShaderNode&,
    GenContext& context,
    Shader& shader) const
{
    ...
    for (const ShaderNode* childNode : _rootGraph->getNodes()) {
        childNode->getImplementation().createVariables(
            *childNode, context, shader);
    }
}

void CompoundNodeSlang::emitFunctionDefinition(
    const ShaderNode&,
    GenContext& context,
...
void CompoundNodeSlang::emitFunctionCall(
